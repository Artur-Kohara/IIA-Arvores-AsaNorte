...
- tentativa opcional de baixar um recorte pequeno da Asa Norte via WMS;
...
2. Baixe as ortofotos GeoTIFF da Asa Norte no Geoportal IDE-DF, conforme indicado no PDF do projeto, ou use a secao opcional `Download de recorte via WMS` do notebook para tentar gerar um recorte pequeno em `data/raw/asa_norte_setor_01.tif`. A rotina WMS nao sobrescreve um arquivo existente por padrao.
...
Cada ortofoto gera sua propria pasta de tiles:
...
A validacao do notebook considera apenas a pasta da ortofoto atualmente processada e verifica dimensoes, CRS e transform georreferenciado dos tiles.
